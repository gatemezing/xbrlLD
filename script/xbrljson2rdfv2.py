...
FRPT = rdflib.Namespace("http://data.mondeca.com/id/financialreport/")
XBRLL = rdflib.Namespace("https://w3id.org/vocab/xbrll#")
ROV = rdflib.Namespace("http://www.w3.org/ns/regorg#")
VCARD = rdflib.Namespace("http://www.w3.org/2006/vcard/ns#")
FRPT = rdflib.Namespace("http://data.mondeca.com/id/financialreport/")
ISO4217 = rdflib.Namespace("http://www.xbrl.org/2003/iso4217")
...
OIM = rdflib.Namespace("https://w3id.org/vocab/oim#")
...
json_data = open('repsol2015semester1.json')
	...
	for j in tuple:
		...
		if j == u'oim:concept': # here for the entity
			...
			if w == "dgi-est-gen:IdentifierValue":
				...
				gxbrl.add((ENT[b], RDF["type"], XBRLL["FinancialEntity"]))
		...
		if j == u'baseType':
			...
			if r == "xs:decimal":
				...
				e1 = tuple[u'oim:unit']
				gxbrl.add((FCT[st], OIM["unitRef"], Literal (e1))) #todo: maybe use the ISO URI
				...
				gxbrl.add((FCT[st], OIM["entity"], ENT[b])) # link to the entity 
			...
			if r == "xs:string":
				...
				if h == "dgi-est-gen:AddressLine":  # metadata and financial report
					...
					gxbrl.add((FRPT[code], RDF["type"], XBRLL["FinancialReport"]))
					gxbrl.add((FRPT[code], DCT["publisher"], ENT[code]))
					gxbrl.add((FRPT[code], RDFS["label"], Literal("Financial report", lang=u'en')))
					gxbrl.add((FRPT[code], RDFS["label"], Literal("Informe financiero", lang=u'es')))

					bn = BNode()

					h2 = tuple[u'oim:period']
					gxbrl.add((FRPT[code], OIM["period"], bn))
				...
				if h == "dgi-est-gen:FormattedName":
					v= tuple[u'value']
					bn1 = BNode()
					gxbrl.add((FRPT[code], XBRLL["contactPerson"], bn1))
...
outfile = open("repsol2015semester1.rdf", "w")
#outfile.write(gxbrl.serialize())
gxbrl.serialize(destination='repsol2015semester1.ttl', format='turtle')
outfile.close()
